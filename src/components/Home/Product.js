...
function Product({ searchValue }) {
  ...
  return (
    <div className='max-w-screen-2xl mx-auto grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 gap-10 px-4 bg-gray-100'>
      {
            ...
            <div key={item.id} className='bg-white h-auto border-[1px] border-gray-200 py-6 z-30 hover:border-transparent shadow-none duration-200 flex flex-col gap-4 relative'>
              ...
              <div>
                <div className='w-full h-auto flex items-center justify-center relative group'>
                  <img className='w-full md:w-36 lg:w-52 h-40 object-contain' src={item.image} alt="product img" />
                  <ul className='w-full h-35 bg-gray-100 absolute bottom-[-157px] flex flex-col items-end justify-center gap-2 font-titleFont px-2 border-1 border-r group-hover:bottom-0 duration-700'>
              ...
              <div className='px-4 z-10 bg-white'>
                <div className='flex items-center justify-between'>
                  <h2 className='font-titleFont tracking-wide text-lg md:text-xl text-amazon_blue font-sm'>{item.title.substring(0,20)}</h2>
                  <p className='text-sm md:text-base text-gray-600 font-semibold'>${item.price}</p>
                </div>
                <div>
                  <p className='text-sm md:text-base'>{item.description.substring(0,100)}...</p>
